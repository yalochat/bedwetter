...
module.exports = function addToCollection (route, origOptions) {
  ...
  return function (request, reply) {
    var combinedFieldsArray = (Array.isArray(origOptions.values.formulateId) ? origOptions.values.formulateId : null)

    if (combinedFieldsArray) {
      request.payload.composedId = ''
      _.forEach(combinedFieldsArray, function (value) {
        request.payload.composedId += request.payload[value]
      })
    }
    ...
    var ChildModel = request.collections[associationAttr.collection];
